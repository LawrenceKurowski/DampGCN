...
class Nettack(BaseAttack):
    ...
    def __init__(self, model, nnodes, nclass,attack_structure, attack_features, device,damping):
        super(Nettack, self).__init__(model, nnodes,attack_structure, attack_features, device,damping)
        self.damping = damping
        if self.damping >0:
            self.lbda1 = model.damp1.lbda_out
            self.lbda2 = model.damp2.lbda_out
#         print(self.lbda1)
#         print(self.lbda2)
    ...
    def get_linearized_weight(self):
        ...
        if self.damping==0:
            ...
            W1 = surrogate.gc1.weight
            W2 = surrogate.gc2.weight
            W1fc = surrogate.fc1.weight
            W2fc = surrogate.fc2.weight
            if isinstance(surrogate.damp1.lbda_out,float):#==<type 'float'>:
                return W1.detach().cpu().numpy(), W2.detach().cpu().numpy(), W1fc.detach().cpu().numpy(), W2fc.detach().cpu().numpy(),surrogate.damp1.lbda_out,surrogate.damp2.lbda_out# lbda1.detach().cpu().numpy(),lbda2.detach().cpu().numpy()
            else:
#                 lbda1 = surrogate.damp1.lbda_out
#                 lbda2 = surrogate.damp2.lbda_out
                return W1.detach().cpu().numpy(), W2.detach().cpu().numpy(), W1fc.detach().cpu().numpy(), W2fc.detach().cpu().numpy(), surrogate.damp1.lbda_out.detach().cpu().numpy(),surrogate.damp2.lbda_out.detach().cpu().numpy()
            
        

    def attack(self, features, adj, labels, target_node, n_perturbations, direct=True, n_influencers= 0, ll_cutoff=0.004, verbose=False, **kwargs):
        ...
        fgsm : float
            >0 means fgsm attack on the node with epsilon = fgsm
        ...
        if self.damping==0:
            ...
            self.W1, self.W2, self.W1fc, self.W2fc,lbda1,lbda2 = self.get_linearized_weight()
            
            if isinstance(lbda1,float):
                first_layer_output = (self.adj_norm@self.modified_features@self.W1 - lbda1*self.modified_features)@ self.W1fc.T
                second_layer_output = ((self.adj_norm@first_layer_output@self.W2) - lbda2*first_layer_output )@ self.W2fc.T
            else:
                first_layer_output = (self.adj_norm@self.modified_features@self.W1 - lbda1@self.modified_features)@ self.W1fc.T
                second_layer_output = ((self.adj_norm@first_layer_output@self.W2) - lbda2@first_layer_output )@ self.W2fc.T
            logits = second_layer_output[target_node]
            logits = np.array(logits)

            if isinstance(lbda1,float):
                logits = np.reshape(logits,(logits.shape[1]),)
...
#         print(type(surrogate_losses))
#         print(len(surrogate_losses))
#         exit()
        if verbose:
        ...
        if len(self.influencer_nodes) == 0:
            if not direct:
                ...
                scores, infls, add_infls  = self.get_attacker_nodes(n_influencers, add_additional_nodes=True)
        ...
        for _ in range(n_perturbations):
            ...
            if attack_structure:
                ...
                indices_of_filtered_edges = filtered_edges_final[:,1]
                self.indices_of_filtered_edges = indices_of_filtered_edges
                # Compute new entries in A_hat_square_uv
                a_hat_uv, a_hat_uv_sq = self.compute_new_a_hat_uv(filtered_edges_final, target_node)

                # Compute the struct scores for each potential edge
                if self.damping==0:
                    struct_scores = self.struct_score(a_hat_uv_sq, self.modified_features @ self.W)
                else:
                    struct_scores = self.struct_score_PS(a_hat_uv,a_hat_uv_sq)
            ...
            if change_structure:
                ...
                best_feature_ix = np.array(best_feature_ix)
#                 print(best_feature_ix.size)
#                 if best_feature_ix.size<2:
# #                 if best_feature_ix ==0:
#                     best_feature_ix = np.array([0,0])
#                     best_feature_ix = np.reshape(best_feature_ix,(2,))#np.array([0,0])
#                 if type(best_feature_ix) is None:
#                     best_feature_ix = np.array([0,0])
#                     best_feature_ix = np.reshape(best_feature_ix,(2,))#np.array([0,0])
        ...
        return np.exp(surrogate_losses)
    ...
    def get_attacker_nodes(self, n=5, add_additional_nodes = False):
        ...
        a_hat_uv,a_hat_uv_sq = self.compute_new_a_hat_uv(potential_edges, self.target_node)
        ...
        if len(neighbors) >= n:  # do we have enough neighbors for the number of desired influencers?
            ...
            if add_additional_nodes:  # Add additional influencers by connecting them to u first.
                ...
                a_hat_uv_additional,a_hat_uv_additional_sq = self.compute_new_a_hat_uv(possible_edges, self.target_node)
                if self.damping==0:
                    additional_struct_scores = self.struct_score(a_hat_uv_additional, XW)
                else:
                    additional_struct_scores = self.struct_score(a_hat_uv_additional, a_hat_uv_additional_sq)
                additional_influencers = poss_add_infl[np.argsort(additional_struct_scores)[-n_additional_attackers::]]
        
                return np.exp(surrogate_losses),influencer_nodes, additional_influencers
            else:
                return np.exp(surrogate_losses),influencer_nodes

    def compute_logits(self):
        if self.damping==0:
            return (self.adj_norm @ self.adj_norm @ self.modified_features @ self.W)[self.target_node]
        else:
            B,C,D= self.add_ps_terms_for_score()
            return (B+self.adj_norm @ C + self.adj_norm @ self.adj_norm @ D )[self.target_node]
...
#         if len(nnz_ixs)<1:
#             nnz_ixs = np.array([0,0])
        sorting = np.argsort(gradients_flipped[tuple(nnz_ixs.T)]) #np.ravel(output)
        sorting = np.ravel(sorting)     
        
        # reshaping for polblogs (uncomment if running for polblogs)
#         nnz_ixs=nnz_ixs.reshape(nnz_ixs.shape[1],)
#         print(nnz_ixs)
#         print(type(nnz_ixs))
#         print(nnz_ixs.shape)
#         print(sorting)
#         print(type(sorting))
#         print(sorting.shape)
#         sorting = np.reshape(sorting,(len(sorting),1))
#         print(sorting.shape)
#         print(sorting)
#         print(nnz_ixs)
        ...
        surrogate_loss = np.repeat(surrogate_loss,grads.shape[1])
#         print(surrogate_loss)
#         print(type(surrogate_loss))
...
#         exit()
    ...
    def gradient_wrt_x(self, label):
        if self.damping==0:
            ...
            W1, W2, W1fc, W2fc,lbda1,lbda2 = self.get_linearized_weight()

            if isinstance(lbda1,float):
                iden = np.eye(self.adj_norm.shape[0]).reshape(-1,1)
                B = iden[self.target_node].dot((lbda1*lbda2 * W1fc.T @ W2fc.T)[:,label].reshape(-1,1).T)
                C = self.adj_norm[self.target_node].T.dot((lbda2*W1@W1fc.T@W2fc.T + lbda1*W1fc.T@W2@W2fc.T)[:,label].reshape(-1,1).T)
                D = self.adj_norm.dot(self.adj_norm)[self.target_node].T.dot((W1@W1fc.T@W2@W2fc.T)[:,label].reshape(1, -1))
 
            else:
                B = (lbda2@lbda1)[self.target_node].reshape(-1,1).dot((W1fc.T @ W2fc.T)[:,label].reshape(-1,1).T)
                
                C = (lbda2@self.adj_norm)[self.target_node].T.reshape(-1,1).dot((W1@W1fc.T@W2fc.T)[:,label].reshape(-1,1).T) + (self.adj_norm@lbda1)[self.target_node].T.reshape(-1,1).dot((W1fc.T@W2@W2fc.T)[:,label].reshape(-1,1).T) 
    ...
    def struct_score_PS(self, a_hat_uv, a_hat_uv_sq):
        """
        Compute structure scores, cf. Eq. 15 in the paper
        ...
        array = np.array(range((B.shape[0])))
        indices_to_delete = np.delete(array,self.indices_of_filtered_edges)
        B = np.delete(B, indices_to_delete,0)
        
        logits = B+a_hat_uv.dot(C) + a_hat_uv_sq.dot(D)
        
        label_onehot = np.eye(logits.shape[1])[self.label_u]
        best_wrong_class_logits = (logits - 1000 * label_onehot).max(1)
        logits_for_correct_class = logits[:,self.label_u]
        struct_scores = logits_for_correct_class - best_wrong_class_logits
    ...
    def add_ps_terms_for_score(self):
        
        W1, W2, W1fc, W2fc,lbda1,lbda2 = self.get_linearized_weight()
        X = self.modified_features
        if isinstance(lbda1,float):
            B = lbda1*lbda2 * X @ W1fc.T @ W2fc.T 
            #"order one" term pre-multiplied by A_hat
            C = -lbda2*X@W1@W1fc.T@W2fc.T - lbda1*X@W1fc.T@W2@W2fc.T
            #"order 2" term pre-multiplied by A_hat^2
            D = X@W1@W1fc.T@W2@W2fc.T
        else:
            B = lbda2@lbda1 @ X @ W1fc.T@ W2fc.T
            #"order one" term pre-multiplied by A_hat
            C = -lbda2@X@W1@W1fc.T@W2fc.T - lbda1@X@W1fc.T@W2@W2fc.T
            #"order 2" term pre-multiplied by A_hat^2
            D = X@W1@W1fc.T@W2@W2fc.T
    ...
    def compute_new_a_hat_uv(self, potential_edges, target_node):
        ...
        ixs, vals_nosq,vals = compute_new_a_hat_uv(edges, node_ixs, edges_set, twohop_ixs, values_before, degrees,
                                         potential_edges.astype(np.int32), target_node)
        ixs_arr = np.array(ixs)
        a_hat_uv_sq = sp.coo_matrix((vals, (ixs_arr[:, 0], ixs_arr[:, 1])), shape=[len(potential_edges), self.nnodes])
        a_hat_uv = sp.coo_matrix((vals_nosq, (ixs_arr[:, 0], ixs_arr[:, 1])), shape=[len(potential_edges), self.nnodes])
        

        return a_hat_uv, a_hat_uv_sq
...
def compute_new_a_hat_uv(edge_ixs, node_nb_ixs, edges_set, twohop_ixs, values_before, degs, potential_edges, u):
    ...
    for ix in range(len(potential_edges)):
        ...
        for v in affected_nodes:
            ...
            return_nosq.append(a_uv_after*mult_term)
            
            return_ixs.append((ix, v))
            return_values.append(new_val)

    return return_ixs, return_nosq,return_values
