...
def label_getter(name,lbda,velocity,veltrain,nodes,topbottom):
        
    label = str('Nettack_lbda='+str(lbda)+'_trainLbda='+str(train_lbda)+'_surrogate_'+name+'_random_nodes='+str(nodes)+'topbottom='+str(topbottom))

    return label

def saver(to_save,name,location,lbda,train_lbda,nodes,topbottom):
    df = pd.DataFrame(to_save)
    to_save_label = label_getter(str(name),lbda,train_lbda,nodes,topbottom)
    df.to_csv(location+to_save_label+'.csv', index=False)


def attack_fgsm(features,adj,target_node,fgsm):
        
#         modified_adj = to_scipy(adj).tolil()
#         modified_features = to_scipy(features).tolil()
        
    modified_features = torch.clone(features)
#     modified_features = sparse_mx_to_torch_sparse_tensor(modified_features)
# #         attack_structure = self.attack_structure

# #         if attack_features:
#     modified_features.to_dense()
    modified_features[target_node] = modified_features[target_node] + fgsm*sign_data_grad[target_node]
    modified_features[target_node] = torch.clamp(modified_features[target_node],0,1)
    modified_features[target_node] = torch.round(modified_features[target_node])
...
def to_sparse(dense_tensor):
    """ Convert a non-sparse tensor to a sparse tensor """
    """https://discuss.pytorch.org/t/how-to-convert-a-dense-matrix-to-a-sparse-one/7809/2"""
    # get indices of non-zero entries (the only ones we will keep)
    indices = torch.nonzero(dense_tensor).t()
    
    # get the values of the non-zero entries
    values = dense_tensor[tuple(indices[i] for i in range(indices.shape[0]))]
    
    # create the sparse tensor with "values" in places of "indices":
